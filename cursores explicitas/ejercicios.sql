...
BEGIN
	...
	WHILE CONSULTA%FOUND LOOP
		IF FILA.EXISTENCIAS < FILA.BAJO_MINIMO THEN
		   DBMS_OUTPUT.PUT_LINE('DEL ARTÍCULO '||FILA.ARTICULO||' '||FILA.PROVEEDOR ||' HAY'||FILA.BAJO_MINIMO - EXISTENCIAS ||'EXISTENCIAS DE MENOS RESPECTO AL MÍNIMO');
		ELSIF FILA.EXISTENCIAS > FILA.SOBRE_MAXIMO THEN
			DBMS_OUTPUT.PUT_LINE('DEL ARTÍCULO '||FILA.ARTICULO||' '||FILA.PROVEEDOR ||' HAY '||FILA.EXISTENCIAS - FILA.SOBRE_MAXIMO||'EXISTENCIAS DE MÁS RESPECTO AL MÄXIMO');
		ELSE
			DBMS_OUTPUT.PUT_LINE('EL ARTICULO '||FILA.ARTICULO||' '||FILA.PROVEEDOR||' ESTA EN EL RANGO DE EXISTENCIAS MÍNIMO_ MÁXIMO');
		END IF;
...
-- 5. Crear la tabla Mis_articulos a imagen de la tabla artículos del usuario almacén, con filas incluidas.

CREATE TABLE MIS_ARTICULOS
AS SELECT *
FROM ARTICULOS;

-- 6. Dada la tabla Mis_articulos, declarar un cursor que devuelva los artículos de los suministradores de Madrid y de Barcelona.

-- Compruebe si el valor de la columna Existencias de cada artículo es menor que el valor asignado en la columna Bajo_mínimo, en cuyo caso se le asignará dicho valor.

-- Sacar por pantalla el número de artículos actualizados.
DECLARE
	CURSOR CONSULTA IS
		...
		WHERE EXISTS(SELECT *
					 ...
					 AND UPPER(PROVINCIAS.DESCRIPCION) IN ('BARCELONA', 'MADRID'));
	FILA CONSULTA%ROWTYPE;
	CANTIDAD NUMBER(8);
BEGIN
	OPEN CONSULTA;
	...
	DBMS_OUTPUT.PUT_LINE('HAN SIDO ACTUALIZADAS '||CANTIDAD||' ARTICULOS');
	
EXCEPTION
	WHEN DUP_VAL_ON_INDEX THEN
		 DBMS_OUTPUT.PUT_LINE('HAS INTENTADO INSERTAR UN DUPLICADO DE LA CLAVE PRIMARIA');
		 DBMS_OUTPUT.PUT_LINE('HAN SIDO ACTUALIZADAS '||CANTIDAD||' ARTICULOS');
END;
