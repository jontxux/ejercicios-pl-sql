...
-- ü Cursor FOR UPDATE.
CREATE OR REPLACE PROCEDURE PRECIO_ARTI(NUMEXISTENCIAS MIS_ARTICULOS.EXISTENCIAS%TYPE)
IS
	CURSOR CONSULTA IS
		SELECT PR_VENT, PR_COST
		FROM MIS_ARTICULOS
		WHERE EXISTENCIAS > NUMEXISTENCIAS
		FOR UPDATE;

	ACT NUMBER(5) := 0;
	BOR NUMBER(5) := 0;
BEGIN
	FOR VAR_LISTA IS NULL AND CONSULTA LOOP
	  IF VAR_LISTA.PR_COST IS NULL AND VAR_LISTA.PR_VENT IS NULL THEN
	       DELETE FROM MIS_ARTICULOS
	       WHERE CURRENT OF CONSULTA;
	       BOR := BOR + 1;

	  ELSIF VAR_LISTA.PR_COST IS NULL THEN
	      ...
	      ACT := ACT + 1;
	  ELSIF VAR_LISTA.PR_VENT IS NULL THEN
	      UPDATE MIS_ARTICULOS
	      SET PR_VENT = PR_COST * 1.05
	      WHERE CURRENT OF LISTA_ARTICULOS;

	      ACT := ACT + 1;
	  END IF;
	END LOOP;
	COMMIT;
	DBMS_OUTPUT.PUT_LINE('FILAS ACTUALIZADAS '||ACT);
	DBMS_OUTPUT.PUT_LINE('ARTICULOS BORRADOS'|| BOR);
	
END PRECIO_ARTI;
	    
/
-- ü ROWID
CREATE OR REPLACE PROCEDURE PRECIO_ARTI(NUMEXISTENCIAS IN MIS_ARTICULOS.EXISTENCIAS%TYPE)
IS
	CURSOR LISTA_ARTICULOS IS
	       SELECT PR_VENT, PR_COST, ROWID
	       FROM MIS_ARTICULOS
	       WHERE EXISTENCIAS > NUMEXISTENCIAS
	       FOR UPDATE;
	NUMFILASACTUALIZADAS NUMBER := 0;
	NUMARTICULOSBORRADOS NUMBER := 0;
BEGIN

	FOR VAR_LISTA IN LISTA_ARTICULOS LOOP
	    IF VAR_LISTA.PR_COST IS NULL AND VAR_LISTA.PR_VENT IS NULL THEN
	       DELETE FROM MIS_ARTICULOS
	       WHERE ROWID = VAR_LISTA.ROWID;

	       NUMARTICULOSBORRADOS := NUMARTICULOSBORRADOS + 1;
	   ELSIF VAR_LISTA.PR_COST IS NULL THEN
	       UPDATE MIS_ARTICULOS
	       SET PR_COST = PR_VENT
	       WHERE ROWID = VAR_LISTA.ROWID;

	       NUMFILASACTUALIZADAS := NUMFILASACTUALIZADAS + 1;
	   ELSIF VAR_LISTA.PR_VENT IS NULL THEN
	       UPDATE MIS_ARTICULOS
	       SET PR_VENT = PR_COST * 1.05
	       WHERE ROWID = VAR_LISTA.ROWID;
	...
	DBMS_OUTPUT.PUT_LINE('FILAS ACTUALIZADAS '||NUMFILASACTUALIZADAS);
	DBMS_OUTPUT.PUT_LINE('ARTICULOS BORRADOS'||NUMARTICULOSBORRADOS);
END PRECIO_ARTI;
...
-- ü FOR UPDATE.
CREATE OR REPLACE PROCEDURE FECHAS_ARTI(DESCRIPCIONPROVINCIA IN PROVINCIAS.DESCRIPCION%TYPE
       	  	  	    		        NUMEROARTICULOSACTUALIZADOS OUT NUMBER,
					        NUMEROARTICULOSBORRADOS OUT NUMBER)
IS
	CURSOR LISTA_ARTICULOS IS

	SELECT FEC_ULT_ENT, FEC_ULT_SAL
	...
	AND UPPER(P.DESCRIPCION) = UPPER(DESCRIPCIONPROVINCIA)
...
FOR VAR_LISTA IN LISTA_ARTICULOS LOOP
    IF VAR_LISTA.FEC_ULT_ENT IS NULL AND VAR_LISTA.FEC_ULT_SAL IS NULL THEN
       DELETE FROM MIS_ARTICULOS
       WHERE CURRENT OF LISTA_ARTICULOS;

       NUMEROARTICULOSBORRADOS := NUMEROARTICULOSBORRADOS + 1;
   ELSIF VAR_LISTA.FEC_ULT_ENT IS NULL THEN
       UPDATE MIS_ARTICULOS
       SET FEC_ULT_ENT = TRUNC(SYSDATE)
       WHERE CURRENT OF LISTA_ARTICULOS;

       NUMEROARTICULOSACTUALIZADOS := NUMEROARTICULOSACTUALIZADOS +1 ;

   ELSIF VAR_LISTA.FEC_ULT_SAL IS NULL
       UPDATE MIS_ARTICULOS
   ...
   ELSIF VAR_LISTA.FEC_ULT_ENT > VAR_LISTA.FEC_ULT_SAL THEN
       UPDATE MIS_ARTICULOS
...
DECLARE
    DESCRI_PROV PROVINCIAS.DESCRIPCION%TYPE  := '&DESCRI';
    BORRADOS NUMBER(5) := 0;
    ACTUALIZADOS NUMBER(5);
BEGIN
    FECHAS_ARTI(DESCRI_PROV, ACTUALIZADOS, BORRADOS);
...
END;
-- ü ROWID.
CREATE OR REPLACE PROCEDURE FECHAR_ARTI(DESCRIPCIONPROVINCIA IN PROVINCIAS.DESCRIPCION%TYPE
       	  	  	    		        NUMEROARTICULOSACTUALIZADOS OUT NUMBER,
					        NUMEROARTICULOSBORRADODS OUT NUMBER)
IS
CURSOR LISTA_ARTICULOS IS
       SELECT FEC_ULT_ENT, FEC_ULT_SAL, MIS_ARTICULOS.ROWID
       FROM MIS_ARTICULOS, PROVEEDORES, PROVINCIAS
       WHERE MIS_ARTICULOS.PROVEEDOR = PROVEEDORES.PROVEEDOR
       AND PROVEEDORES.PROVINCIA = PROVINCIAS.PROVINCIA
       AND UPPER(P.DESCRIPCION) = UPPER(DESCRIPCIONPROVINCIA
...
BEGIN
	   ...
	   NUMEROARTICULOSBORRADOS := NUMEROSARTICULOSBORRADOS + 1;
           ELSIF VAR_LISTA.FEC_ULT_ENT IS NULL THEN
	   UPDATE MIS_ARTICULOS
	   SET FEC_ULT_ENT = TRUNC(SYSDATE)
	   WHERE ROWID = VAR_LIST.ROWID;

	   NUMEROARTICULOSACTUALIZADOS := NUMEROARTICULOSACTUALIZADOS + 1;
           ELSIF VAR_LISTA.FEC_ULT_SAL IS NULL THEN
	   UPDATE MIS_ARTICULOS
	   SET FEC_ULT_SAL = SYSDATE
	   WHERE ROWID = VAR_LIST.ROWID;
	   ...
	   NUMEROARTICULOSACTUALIZADOS := NUMEROSARTICULOSACTUALIZADOS + 1;
           END IF;
END LOOP;
END FECHAS_ARTI;
-- ü Sin ninguno de las anteriores.
...
CREATE OR REPLACE PROCEDURE ALTA_FORMA(COD IN FORMPAGOS.FORMPAGO%TYPE, DESCRI IN FORMPAGOS.DESCRIPCTION%TYPE)
IS
    INF VARCHAR2(50);
