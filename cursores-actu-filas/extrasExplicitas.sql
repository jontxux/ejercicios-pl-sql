...
IS
	CURSOR CONSULTA IS
		SELECT PROVEEDORES.PROVEEDOR, EMPRESA AVG(NVL(PR_COST), 0)
		FROM PROVEEDORES, ARTICULOS
		WHERE ARTICULOS.PROVEEDOR = PROVEEDORES.PROVEEDOR
		GROUP BY PROVEEDORES.PROVEEDOR, EMPRESA;
...
BEGIN
	...
	FETCH CONSULTA INTO FILA;
	WHILE CONSULTA%FOUND LOOP
		DBMS_OUTPUT.PUT_LINE('EL PROVEEDOR '||FILA.EMPRESA);
		DBMS_OUTPUT.PUT_LINE('PROMEDIO VENTA '||CANTIDAD||' €');
...
IS
	CURSOR CONSULTA IS
		SELECT PROVINCIA, DESCRIPCION, COUNT(*)
